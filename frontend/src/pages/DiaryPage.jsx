...
const DiaryPage = () => {
  ...
  useEffect(() => {
    ...
    fetchLogs();
  }, [username, currentPage, showLogForm]);
              ...
              return (
                <tr key={log.logId} className="hover:bg-gray-800">
                  ...
                  {authUser?.username === username && (
                    <td className="p-3">
                      <button onClick={() => setEditingLog(log)} className="text-blue-400 cursor-pointer hover:text-blue-300">Edit</button>
                      {/* {showLogForm && <LogForm setshowLogForm={setshowLogForm} liked={log.liked} rating={log.rating} rewatchChecked={log.rewatch} reviewId={log.reviewId} review={log.review} movieId={log.movieId} username={username} logId={log.logId} watchedOnChecked={true}/>} */}
                      {editingLog && editingLog.logId === log.logId && (
                        <LogForm
                      ...
                      {console.log(log.movieId, log.title, username, log.logId, log.liked, log.rating, log.reviewId, log.watchedOn, log.rewatch, log.review, log.watchedOn)}
                    </td>
                  )}
                </tr>
              )
